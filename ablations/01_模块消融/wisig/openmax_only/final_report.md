...
- 输出目录：`D:\learn_pytorch\笔记\方案\os_sei_code\ablations\01_模块消融\wisig\openmax_only`

## 核心指标

| 指标键 | 中文名 | 数值 | 说明 |
| --- | --- | ---: | --- |
| overall_accuracy | 总体准确率 | 0.976562 | 已知类分对且未知类拒识正确的总比例，越高越好。 |
| known_accuracy | 已知类准确率 | 0.888672 | 只看真实已知类样本，被分到正确已知类别的比例，越高越好。 |
| macro_f1 | 宏平均 F1 | 0.943172 | 每个类别 F1 的平均值，更关注类别均衡表现，越高越好。 |
| auroc | 已知/未知区分 AUROC | 0.987252 | unknown score 区分已知与未知的整体能力，越接近 1 越好。 |
| fpr95 | 95% 未知召回下的已知误拒率 | 0.038672 | 未知召回约 95% 时，已知样本被误拒为 unknown 的比例，越低越好。 |
...
| 指标键 | 中文名 | 数值 | 说明 |
| --- | --- | ---: | --- |
| macro_precision | 宏平均精确率 | 0.998304 | 每个类别 precision 的平均值，越高越好。 |
| macro_recall | 宏平均召回率 | 0.895221 | 每个类别 recall 的平均值，越高越好。 |
| weighted_f1 | 加权 F1 | 0.975934 | 按类别样本数加权后的 F1，越高越好。 |
| unknown_precision | 未知类精确率 | 0.971168 | 被拒识为 unknown 的样本中真实未知类占比，越高越好。 |
| known_fpr_as_unknown | 已知类误拒率 | 0.111328 | 真实已知类被错误拒识成 unknown 的比例，越低越好。 |
| unknown_false_accept_rate | 未知类误接收率 | 0.000000 | 真实未知类被错误接受为某个已知类的比例，越低越好。 |
| oscr | 开放集分类识别曲线面积 | 0.888642 | 同时考虑已知类分类正确率和未知拒识能力的综合面积，越高越好。 |
...
- 图表目录：D:\learn_pytorch\笔记\方案\os_sei_code\ablations\01_模块消融\wisig\openmax_only\figures
