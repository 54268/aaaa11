...
def _plot_openness_metric_curve(path: Path, curve_rows: list[Dict[str, float]]) -> None:
    ...
    for key, label, color in [
        ("overall_accuracy", "总体准确率", "#1f4e79"),
        ("known_accuracy", "已知类准确率", "#2b8c56"),
        ("macro_f1", "宏平均 F1", "#e67e22"),
        ("unknown_recall", "未知类召回率", "#c0392b"),
        ("auroc", "AUROC", "#7f3c8d"),
    ]:
        ax.plot(openness, [row[key] for row in curve_rows], marker="o", linewidth=2.1, label=label, color=color)
    ax.set_xlabel("开放度")
    ax.set_ylabel("指标值")
    ax.set_title("开放度-指标曲线")
...
def _plot_accuracy_f1_curve(path: Path, curve_rows: list[Dict[str, float]]) -> None:
    ...
    ax.plot(openness, [row["overall_accuracy"] for row in curve_rows], marker="o", linewidth=2.2, color="#1f4e79", label="总体准确率")
    ax.plot(openness, [row["macro_f1"] for row in curve_rows], marker="s", linewidth=2.2, color="#e67e22", label="宏平均 F1")
    ax.set_xlabel("开放度")
    ax.set_ylabel("指标值")
    ax.set_title("准确率和宏平均 F1 随开放度变化")
    ax.legend(frameon=False, loc="best")
    _paper_axes(ax)
    fig.tight_layout()
    fig.savefig(path, dpi=220)
