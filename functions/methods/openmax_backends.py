...
from functions.methods.repo_openmax import OpenMax, WeibullFitting
...
class RepoOpenMaxAdapter:
    ...
    def fit(
        ...
        fitter = self._WeibullFitting(
            tailsize=self.tail_size,
            num_classes=len(classes),
        )
        distances, centroids = fitter.compute_centroids_and_distances(
            samples,
            distance_type=self.distance_type,
        )
        weibull_models = fitter.fit_all_models(distances)

        self.centroids = [centroid.detach().cpu() for centroid in centroids]
    ...
    def predict(self, activations: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        logits_hat = self._model.recalibrate_logits(
            torch.from_numpy(activations.astype(np.float32))
        )
    ...
    def state_dict(self) -> Dict[str, object]:
        return {
            ...
            "distance_type": self.distance_type,
            "centroids": (
                [centroid.numpy() for centroid in self.centroids]
    ...
    def from_state_dict(cls, state: Dict[str, object]) -> "RepoOpenMaxAdapter":
        ...
        obj.centroids = [
            ...
            for centroid in list(state["centroids"])
        ]
        obj.weibull_models = state["weibull_models"]
        obj._model = obj._OpenMax(
            centroids=[torch.from_numpy(centroid) for centroid in obj.centroids],
