...
def prepare_wisig_compact(config: dict) -> Dict[str, object]:
    ...
    available_equalized = []
    for payload in payloads:
        for rx_name in payload["rx_list"]:
            if rx_name not in available_rx:
                available_rx.append(rx_name)
        for capture_date in payload["capture_date_list"]:
            if capture_date not in available_dates:
                available_dates.append(capture_date)
        for eq_value in payload["equalized_list"]:
            if eq_value not in available_equalized:
                available_equalized.append(eq_value)
    selected_rx = sorted(include_rx if include_rx is not None else available_rx)
    selected_dates = sorted(include_capture_dates if include_capture_dates is not None else available_dates)
    selected_equalized = sorted(include_equalized if include_equalized is not None else available_equalized)
    ...
    summary = {
        ...
        "known_count": int(len(known_classes)),
        "unknown_count": int(len(unknown_classes)),
        "num_tx": int(len(known_classes) + len(unknown_classes)),
        "split_mode": prep_cfg.get("split_mode", ""),
        "fixed_rx": prep_cfg.get("fixed_rx", selected_rx[0] if len(selected_rx) == 1 else None),
        "rx_used": selected_rx,
        "num_rx_used": int(len(selected_rx)),
        "capture_dates_used": selected_dates,
        "equalized_used": selected_equalized,
        ...
        "train_sample_count": int(len(splits["train_idx"])),
        "val_sample_count": int(len(splits["val_idx"])),
        "test_known_sample_count": int(len(splits["test_idx"])),
        "test_unknown_sample_count": int(len(y_unknown)),
