...
class PytorchOodOpenMaxAdapter:
    def __init__(self, alpha_rank: int = 3, tail_size: int = 20, euclid_weight: float = 1.0) -> None:
        repo_src = _project_root() / "third_party" / "pytorch-ood" / "src"
        _register_path(repo_src)
        from pytorch_ood.detector.openmax.numpy import OpenMax  # type: ignore

        self._OpenMax = OpenMax
        self.alpha_rank = alpha_rank
        self.tail_size = tail_size
        self.euclid_weight = euclid_weight
        self._model = self._OpenMax(tailsize=tail_size, alpha=alpha_rank, euclid_weight=euclid_weight)

    def fit(self, activations: np.ndarray, labels: np.ndarray, predictions: np.ndarray) -> None:
        mask = predictions == labels
        fit_x = activations[mask] if np.any(mask) else activations
        fit_y = labels[mask] if np.any(mask) else labels
        self._model.fit(fit_x, fit_y)

    def predict(self, activations: np.ndarray) -> Dict[str, np.ndarray]:
        probs = self._model.predict(activations)
        return {
            "known_probs": probs[:, 1:],
            "unknown_prob": probs[:, 0],
            "pred": probs[:, 1:].argmax(axis=1),
            "revised_activations": probs[:, 1:],
        }
    ...
    def state_dict(self) -> Dict[str, object]:
        return {
            ...
            "distributions": self._model.distributions,
            "backend": "pytorch_ood",
        }

    @classmethod
    def from_state_dict(cls, state: Dict[str, object]) -> "PytorchOodOpenMaxAdapter":
        obj = cls(
            alpha_rank=int(state["alpha_rank"]),
            tail_size=int(state["tail_size"]),
            euclid_weight=float(state["euclid_weight"]),
        )
        obj._model.n_dims = state["n_dims"]
        obj._model.centers = state["centers"]
        obj._model.distributions = state["distributions"]
        obj._model.is_fitted = True
        return obj
