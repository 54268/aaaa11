...
from augmentations import gen_aug1
from utils import tsne, mds, _logger
import time
from models.frameworks import *
from models.backbones import *
from models.loss import *
from data_preprocess import data_preprocess_RF_mini_10
...
def setup_dataloaders(args):
    if args.dataset == 'RF_mini_10':
        args.height = 2
        args.transformer_embedding_size = 64
        args.width = 1024
        args.n_feature = args.height * args.transformer_embedding_size
        args.len_sw = args.width // args.transformer_embedding_size + 1
        args.n_class = 10
        train_loaders, val_loader, test_loader = data_preprocess_RF_mini_10.prep_rf(args,SLIDING_WINDOW_LEN=args.len_sw,SLIDING_WINDOW_STEP=int(args.len_sw * 0.5))
...
def setup_model_optm(args, DEVICE, classifier=True):
    ...
    elif args.backbone == 'RoInformer':
        backbone = RoInformer(n_channels=args.n_feature, len_sw=args.len_sw, n_classes=args.n_class, dim=128,heads=4,d_ff=512, dropout=0.1, backbone=False)
    else:
        NotImplementedError
...
def setup(args, DEVICE):
    ...
    if args.criterion == 'cos_sim':
        ...
        if args.framework == 'tstcc':
            ...
            criterion = NTXentLoss(DEVICE, args.batch_size, temperature=0.05)
...
def calculate_model_loss(args, sample, target, model, criterion, DEVICE, recon=None, nn_replacer=None):
    if  args.aug1 =='same' and args.aug2 =='repeat':
        aug_sample1 = gen_aug1(args, sample, target, args.aug1)
        aug_sample2 = gen_aug1(args, sample, target, args.aug2)
    else:
