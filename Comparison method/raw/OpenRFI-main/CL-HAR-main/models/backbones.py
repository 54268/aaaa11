...
class RoInformer(nn.Module):
    def __init__(self, n_channels, len_sw, n_classes, dim=128,  heads=4, d_ff=512, dropout=0.1, backbone=True):
        super(RoInformer, self).__init__()

        self.backbone = backbone
        self.out_dim = dim
        self.roinformer = Seq_RoInformer(n_channel=n_channels, len_sw=len_sw, n_classes=n_classes, dim=dim,  heads=heads,d_ff=d_ff, dropout=dropout)
        if backbone == False:
            self.classifier = nn.Linear(dim, n_classes)
