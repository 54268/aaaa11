...
equalized = 0
dataset_path = 'H:/RF fingerprint/SingleDay.pkl/'
...
class CNNFeatureExtractor(nn.Module):
    def __init__(self, input_channels=2, d_model=64):
        ...
        self.layers = nn.Sequential(
            ...
            ResConv1d(32, 32, kernel_size=3, dilation=3),
...
# Example: Load the model and compute probabilities for a sample
loaded_model = CNNTransformerModel(input_dim, num_classes).to(device)
loaded_model.load_state_dict(torch.load('model.pth'))

# Take the first test sample
X_sample = X_test[0:1].to(device)
y_true_sample = y_test[0].argmax().item()  # True class index
probabilities = loaded_model.predict_proba(X_sample).cpu().numpy()[0]

print(f"\nTrue class: {tx_list[y_true_sample]}")
print("Predicted probabilities for each category:")
for i, prob in enumerate(probabilities):
    print(f"{tx_list[i]}: {prob:.4f}")