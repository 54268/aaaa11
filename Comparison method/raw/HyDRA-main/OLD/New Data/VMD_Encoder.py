...
# VMD preprocessing
def Dataset_VMD(dataset, alpha, tau, k, DC, init, tol):
    ...
    for dat in dataset:
        adc = dat[:, 0] + dat[:, 1]
        u, _, _ = VMD(adc, alpha, tau, k, DC, init, tol) 
...
X_train = Dataset_VMD(X_train, alpha, tau, k, DC, init, tol)
X_val = Dataset_VMD(X_val, alpha, tau, k, DC, init, tol)
X_test = Dataset_VMD(X_test, alpha, tau, k, DC, init, tol)
