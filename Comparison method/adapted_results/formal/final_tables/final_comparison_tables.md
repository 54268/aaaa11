...
本文件为 formal 结果目录下的精简版最终汇总，只保留对比方法最核心的拒识与细分指标。开放集拒识重点汇总 `unknown_recall`、`known_accuracy`、`macro_f1`、`auroc`；未知类细分重点汇总 `nmi`、`ari`、`hungarian_accuracy` 与覆盖率 `coverage_of_total_test_unknown`。
