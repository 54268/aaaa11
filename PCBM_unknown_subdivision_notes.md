...
以当前 Oracle 与 WiSig 的统一细分管线为准，完整流程如下。两者共享 `embedding_stats + PCA96 + unknown-only GMM-full-direct` 口径；GMM 候选分量数统一写为 `K+m`，其中 `K` 为协议给定未知类数，`m∈{0,1,2}` 为小范围冗余候选因子。Oracle 因未知类混合更明显，主结果取 `m=2`；WiSig 当前未知类更干净，主结果取 `m=0`。
...
本文只在小范围 `m∈{0,1,2}` 中固定主实验设置。Oracle 选择 `m=2`，因为 `m=1` 在当前特征空间中仍容易出现真实未知类合并，而 `m=2` 能恢复 6 个稳定有效簇，并保持约 84% 的未知样本覆盖率。WiSig 选择 `m=0`，因为当前 SingleDay 协议下未知类已经非常干净，额外冗余分量会把部分真实未知类拆成类内子模态。该设置不是用真实未知标签在线自动选择，而是作为实验协议中的小范围超参数固定下来，并可在消融中报告。
