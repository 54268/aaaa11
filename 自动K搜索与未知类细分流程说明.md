...
本文档专门说明当前 PCBM 未知类细分阶段如何自动搜索候选 K，以及整体细分流程相对历史给定 K 诊断口径发生了什么变化。
...
历史诊断实验会把协议中的真实未知类数作为目标 K，再额外搜索缓冲分量。这个设置便于排查聚类后端和特征组合，但不符合真实开放场景，因为实际部署时通常不知道未知发射源数量。
...
```text
本文方法不预先给定未知类数，而是自动选择 GMM 候选分量数。最终有效未知簇数由 GMM 候选筛选和簇规模均衡合并共同决定。
```

避免写法：

```text
给定真实未知类数 K 后运行缓冲分量搜索。
```

这个说法只适合历史诊断实验，不适合当前主结果。
